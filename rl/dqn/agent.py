...
from buffer.experience_history import ExperienceHistory, sqilExperienceHistory
...
class DQN:
    ...
    def __init__(self,
        ...
        self.algo_name = "rl_dqn"
        if(soft_dqn):
            self.algo_name += "_soft"
        self.soft_dqn = soft_dqn
        self.sqil = sqil
        if sqil:
            self.exp_history = sqilExperienceHistory(
    ...
    def build_graph(self):
        ...
        with tf.variable_scope("fixed", reuse=tf.AUTO_REUSE):
        ...
        if(self.soft_dqn):
            q_target = tf.reduce_logsumexp(qsa_targets, -1) * self.gamma * not_done + self.input_reward
        else:
    ...
    def create_network(self, input, trainable):
        if trainable:
            wr = tf.keras.regularizers.L2(self.regularization)
        else:
            wr = None
        ...
        net = tf.layers.conv2d(input_t, 8, (7, 7), data_format="channels_last", activation=tf.nn.relu, strides=3, kernel_regularizer=wr, trainable=trainable)
        net = tf.layers.max_pooling2d(net, 2, 2)
        
        net = tf.layers.conv2d(net, 16, (3, 3), data_format="channels_last", activation=tf.nn.relu, kernel_regularizer=wr, trainable=trainable)
        net = tf.layers.max_pooling2d(net, 2, 2)
        
        net = tf.layers.flatten(net)
        net = tf.layers.dense(net, 256, activation=tf.nn.relu, kernel_regularizer=wr, trainable=trainable)
        
        q_state_action_values = tf.layers.dense(net, self.dim_actions, activation=None, kernel_regularizer=wr, trainable=trainable)
    ...
    def play_episode(self,gen_demos=False):
        eh = (
        ...
        while True:
            ...
            for _ in range(self.frame_skip):
                ...
                reward += r
                if gen_demos:
                    demo.append((self.process_image(observation), action_idx, done, reward))
                if done:
                    break
            ...
            if self.do_training:
                ...
                if(self.sqil):
                    train_cond = train_cond and self.exp_history.check_learner_buffer_size(self.batchsize)
                if train_cond:
                    self.train()
            ...
            if done:
                ...
                return total_reward, frames_in_episode, demo
